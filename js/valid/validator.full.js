(function() {
    ...
    function ValidatorElement(fieldName) {
        ...
        if (el.size() == 0) {
            alert("Element not found" + fieldName + "")
        } else {
        ...
        this.checkSize = function(func) {
            if (this.el.size() > 1) {
                alert("Name property of the same multiple form fields, support only detect whether empty. Not supported'" + func + "()'");
                return false
            }
            return true
        };
        this.setType = function(type, msg, exclude, disabledCallback) {
            if (this.checkSize("setType")) {
                ...
                if (regexp) {
                    ...
                    alert("Absence detection type'" + type + "'")
                }
            }
            return this
        };
        ...
        this.setDisabledCallback = function(callback) {
            if (typeof callback != "function") {
                alert(callback + "Javascript function is not valid")
            }
        ...
        this.displayLoading = function() {
            if (this.showErrorMode != "alert") {
                this.getTipSpan().html('<span class="validatorMsg validatorLoad">Detecting, please wait...</span>')
            }
            return this
        };
    ...
    var FormValidator = {
                    ...
                    $.ajax({
                        ...
                        error: function(XMLHttpRequest, textStatus, errorThrown) {
                            ...
                            if (textStatus == "timeout") {
                                textStatus = "Timeout, please try again. "
                            }
                            FormValidator.forms[vObj.formId].submitClicked = false;
        ...
        reset: function(form) {
            ...
            if (!formEl) {
                alert("'FormValidator.reset():'Not detected form");
                return false
            }
        ...
        addFormCallback: function(form, funcName) {
            var formEl = FormValidator.getForm(form);
            if (!formEl) {
                alert("'FormValidator.addFormCallback():'After testing procedures must be placed in the form");
                return false
            }
            var newForm = this.addForm(formEl);
