...
class mysql
{
    ...
    function mysql($dbhost, $dbuser, $dbpw, $dbname = '', $pconnect = 0, $quiet = 0)
    {
        ...
        if (defined('AWEBCOM_ROOT') && !$this->root_path) {
            $this->root_path = AWEBCOM_ROOT;
        }
        if ($quiet) {
	...
	function connect($dbhost, $dbuser, $dbpw, $dbname = '', $pconnect = 0, $quiet = 0)
    {
        ...
        if ($pconnect) {
            if (!($this->link = @mysql_pconnect($dbhost, $dbuser, $dbpw))) {
                if (!$quiet) {
                    $this->ErrorMsg("Can't Pconnect MySQL Server($dbhost)!<br><a href='http://studioweb.pro'>Click Here</a>");
                }
                return false;
            }
        } else {
