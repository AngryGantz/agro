...
$count = $db->getOne("SELECT COUNT(*) FROM {$table}comment WHERE is_check=1 AND type = '' and infoid = '$infoid'" );
$pager['search'] = array('infoid' => $infoid);
$pager = get_pager('comment.php', $pager['search'], $count, $page, $INF['maxcomment']);
$sql = "SELECT * FROM {$table}comment WHERE infoid = '$infoid' AND is_check = 1 AND type = '' ORDER by id DESC LIMIT $pager[start],$pager[size]";
$res = $db->query($sql);
$comment = array();
